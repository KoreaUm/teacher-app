...
function buildHwpMarkdownPrompt(topic, docType) {
  ...
  return [
    ...
    ``,
    `[출력 규칙 - 반드시 지켜주세요]`,
    `- 아래 태그 형식으로만 작성 (다른 설명 금지)`,
    `- 제목: 문서 제목`,
    `- 서론: 문서 취지와 필요성을 1~2문장`,
    `- 대제목: N: 제목 — 큰 번호 박스. ⚠️반드시 해당 섹션의 본문(□/원/바) 줄들 앞에 한 줄로 위치할 것. 본문 뒤에 두지 말 것`,
    `- 중제목번호: N: 제목 — 작은 번호 박스. 대제목과 동일하게 섹션 본문 앞에 위치`,
    `- 붙임: N: 제목 — "붙임 N" 라벨 박스. 문서 가장 마지막에 한 줄씩 나열`,
    `- 네모: 주요 섹션 제목. 한글 변환 시 파란색 □ 소제목으로 표시`,
    `- 원: 1단계 항목. 한글 변환 시 행안부 문서처럼 ㅇ 항목으로 표시`,
    `- 바: 2단계 항목`,
    `- 별: 3단계 참고 항목. 한글 변환 시 ￭ 항목으로 표시`,
    `- 시간계획표: 시작시간:종료시간: 활동내용 : 비고`,
    `- 표가 필요한 경우 시간계획표 태그를 여러 줄 사용`,
    `- 2025년 행정안전부 업무계획 문서처럼 보고서형 문체 사용: 간결, 격식, 명사형 종결("~함", "~필요", "~추진")`,
    `- 학교/교사 업무 맥락에 맞게 작성`,
    ``,
    `[예시]`,
